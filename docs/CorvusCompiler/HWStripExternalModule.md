...
- Turn the original public module into a private implementation module named
  `corvus_top`.
- Create a private companion module `corvus_external` that contains the original
  instances with their connectivity exposed through ports.
- Generate a public wrapper module `corvus_wrapper_t0` whose signature and
  behaviour match the original `user_top`.
...
2. It renames that module to `corvus_top`, clones it twice inside the same
   `mlir::ModuleOp`, and renames the clones to `corvus_external` and
   `corvus_wrapper_t0`.
3. The original module and `corvus_external` are marked private, while
   `corvus_wrapper_t0` remains public.
...
### Building the Wrapper (`corvus_wrapper_t0`)

1. Start from the `corvus_wrapper_t0` clone, erase all operations in its body
...
- `corvus_top`: a private module containing the original logic with extra ports
  for every instance connection.
- `corvus_external`: a private module comprising the extracted instances and
  matching bridge ports.
- `corvus_wrapper_t0`: a public module that instantiates the previous two
  modules, wires their ports using SSA values (no SystemVerilog ops are
  introduced), and preserves the original IO behaviour of `user_top`.
